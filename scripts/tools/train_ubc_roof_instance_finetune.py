"""Train and evaluate OlmoEarth instance segmentation on UBC v2 (fine-tunable).

Copy of ``train_ubc_roof_instance.py`` that can also fine-tune the encoder.
Pass ``--unfreeze-backbone`` to optimize the encoder with its own (smaller)
learning rate; without the flag the behaviour matches the frozen script.
...
class OlmoEarthPyramid(nn.Module):
    """OlmoEarth encoder plus a trainable four-level feature pyramid."""
    ...
    def __init__(
        ...
        # pretraining decoder would waste roughly another 140M parameters
    ...
    def forward(self, images: torch.Tensor) -> OrderedDict[str, torch.Tensor]:
        ...
        # When fine-tuning, inherit the ambient grad mode so inference paths that
        # already wrap the model in ``torch.no_grad`` stay cheap.
...
def build_model(
    ...
    print(f"loading {state} OlmoEarth backbone from {weights}")
    ...
    backbone = OlmoEarthPyramid(
        olmo, task, patch_size, embedding_size, s2_rgb_mode, unfreeze_backbone
    )
...
def _checkpoint_args(checkpoint: dict[str, Any], args: argparse.Namespace) -> None:
    for name in ("task", "tile_size", "patch_size", "s2_rgb_mode", "unfreeze_backbone"):
...
def train(args: argparse.Namespace) -> None:
    ...
    if encoder_parameters:
        # A pretrained encoder is destroyed by the head learning rate.
