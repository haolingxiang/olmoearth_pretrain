"""UBC roof instance segmentation v4: Hybrid R-CNN.

Replaces Mask R-CNN / Cascade with a query detector:

* no RPN and no dense anchors
* 300 learnable proposals refined by DynamicConv stages
* QueryInst dynamic masks on the same queries
* Seesaw classification for long-tailed roof types

Single-GPU only. Do not --init-ckpt from v3; the heads are a different model.
"""
...
from ubc_cascade_seesaw import collect_class_counts  # noqa: E402
...
def build_model(
    ...
    print(
        ...
        f"(v4 HybridRCNN; seesaw={use_seesaw}, queries={num_queries}, "
        f"stages={num_query_stages})"
    )
    ...
    torch.save(
        {
            ...
            "num_query_stages": getattr(args, "num_query_stages", 6),
...
def train(args: argparse.Namespace) -> None:
    ...
    model = build_model(
        ...
        num_query_stages=args.num_query_stages,
    )
    if args.init_ckpt:
    ...
    print(
        ...
        f"input={args.tile_size} eval_stride={args.stride}"
    )
    print(
        ...
        f"hybrid_rcnn queries={args.num_queries} stages={args.num_query_stages}, "
        f"seesaw={args.seesaw}"
    )
...
def load_for_inference(
    ...
    model = build_model(
        ...
        num_queries=int(checkpoint.get("num_queries", getattr(args, "num_queries", 300))),
        num_query_stages=int(
            checkpoint.get("num_query_stages", getattr(args, "num_query_stages", 6))
        ),
    )
    load_trainable_state(model, checkpoint)
    model.eval()
...
def add_common(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        type=int,
        default=512,
        help="model input size; UBC images are 512 so this is full-image (no 256 crop)",
    )
    ...
    parser.add_argument(
        ...
        type=int,
        default=300,
        help="learnable Hybrid R-CNN queries per image (replaces RPN proposals)",
    )
    parser.add_argument(
        "--num-query-stages",
        type=int,
        default=6,
        help="iterative DynamicConv refinement stages",
    )
...
def add_inference(parser: argparse.ArgumentParser) -> None:
    parser.add_argument(
        ...
        type=int,
        default=0,
        help="eval window stride; 0 means tile-size (no overlap / no 256 sliding)",
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    train_parser = commands.add_parser(
        "train", help="train v4 Hybrid R-CNN (single GPU)"
    )
    ...
    train_parser.add_argument(
        ...
        help=(
            ...
            "v4 default 2 to stress rare roof types"
        ),
    )
    ...
    train_parser.add_argument(
        ...
        type=int,
        default=100,
        help="print a loss line every N batches; tqdm postfix updates every step",
    )
    ...
    train_parser.add_argument(
        "--init-ckpt",
        default=None,
        help="ignored for architecture; HybridRCNN heads do not load v3 Mask/Cascade weights",
    )
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if hasattr(args, "stride"):
        ...
        if not 0 < args.stride <= args.tile_size:
            raise ValueError("expected 0 < --stride <= --tile-size (0 means tile-size)")
    probability_names = (
        "score_threshold",
        "mask_threshold",
        "box_nms",
    ...
    if args.command == "train":
        ...
        if args.num_query_stages < 1:
            raise ValueError("--num-query-stages must be positive")
