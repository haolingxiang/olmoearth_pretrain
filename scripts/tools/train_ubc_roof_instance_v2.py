"""UBC roof instance segmentation v2: stronger neck + detection scale coverage.

Built on ``train_ubc_roof_instance_finetune.py`` with structural upgrades aimed at
the measured failure modes (weak large-object AP, low recall, long-tail classes):

* RGB(/SAR) detail skip fused into the finest pyramid level
* five-level SimpleFPN with top-down refinement (not just successive downsampling)
* larger anchors up to 256 px and higher RPN/proposal budgets
* optional encoder fine-tuning via ``--unfreeze-backbone``
"""
...
def build_model(
    ...
    print(f"loading {state} OlmoEarth backbone from {weights} (v2 neck)")
    ...
    # Cover tiny sheds through large industrial roofs on 512 px tiles.
...
def build_parser() -> argparse.ArgumentParser:
    ...
    train_parser = commands.add_parser("train", help="train v2 Mask R-CNN (detail FPN)")
    ...
    train_parser.add_argument(
        ...
        help=(
            ...
            "v2 default 2 to stress rare roof types"
        ),
    )
