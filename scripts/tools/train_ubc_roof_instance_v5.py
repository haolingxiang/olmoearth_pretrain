"""UBC roof instance segmentation v5: v3 head + P0 eval + RGB adapter.

Keeps v3 Cascade + Seesaw + copy-paste. Adds the minimum robust combo:

* trainable RGB→S2 adapter initialized to the handcrafted repeat/rgb-only map
* layer-wise encoder LRs (top ``--backbone-lr``, decay ``--layerwise-gamma``)
* full-val IDs on disk, class-agnostic AP50, copy-paste off in the last 10% epochs

Single-GPU only. Do not ``--init-ckpt`` from v4 Hybrid weights.
"""
...
from ubc_rgb_adapter import RgbToS2Adapter  # noqa: E402
...
class OlmoEarthPyramid(nn.Module):
    ...
    def __init__(
        ...
        self.rgb_adapter: RgbToS2Adapter | None
        if use_rgb_adapter:
            self.rgb_adapter = RgbToS2Adapter(
                s2_low, s2_high - s2_low, mode=s2_rgb_mode
            )
        else:
    ...
    def forward(self, images: torch.Tensor) -> OrderedDict[str, torch.Tensor]:
        ...
        adapter_trainable = self.rgb_adapter is not None and any(
            parameter.requires_grad for parameter in self.rgb_adapter.parameters()
        )
        context = (
            nullcontext()
            if self.unfreeze_backbone or adapter_trainable
            else torch.no_grad()
        )
        with context:
...
def build_model(
    ...
    print(
        ...
        f"(v5 neck; cascade={use_cascade}, seesaw={use_seesaw}, "
        f"rgb_adapter={use_rgb_adapter}, s2_rgb_mode={s2_rgb_mode})"
    )
    ...
    backbone = OlmoEarthPyramid(
        ...
        use_rgb_adapter=use_rgb_adapter,
    )
...
def _reject_hybrid_checkpoint(checkpoint: dict[str, Any]) -> None:
    if checkpoint.get("hybrid") or checkpoint.get("neck_version") == "v4":
        raise ValueError("v5 cannot load v4 Hybrid weights; use v2/v3 or a v5 ckpt")
    ...
    torch.save(
        {
            ...
            "neck_version": "v5",
            "rgb_adapter": getattr(args, "rgb_adapter", True),
            "layerwise_gamma": getattr(args, "layerwise_gamma", 0.8),
...
def select_eval_images(
    ...
    if image_ids is not None:
        if not image_ids:
            raise ValueError("eval image-id list is empty")
        ordered = [int(image_id) for image_id in image_ids]
        by_id = {int(row["id"]): row for row in images}
        missing = [image_id for image_id in ordered if image_id not in by_id]
        if missing:
            raise ValueError(f"eval image-id list missing {len(missing)} images")
        return [by_id[image_id] for image_id in ordered]
...
def predict_split(
    ...
    model.eval()
    rows = select_eval_images(
        index.images,
        max_images,
        args.seed,
        getattr(args, "eval_image_ids", None),
    )
...
def evaluate_coco(
    ...
    if not predictions:
        return {"AP": 0.0, "AP50": 0.0, "AP75": 0.0, "AP50_building": 0.0}
...
def _class_agnostic_ap50(
    ...
    agnostic_predictions = [{**row, "category_id": 1} for row in predictions]
    agnostic_dt = agnostic_gt.loadRes(agnostic_predictions)
    evaluator = COCOeval(agnostic_gt, agnostic_dt, "segm")
    if image_ids is not None:
        evaluator.params.imgIds = image_ids
    evaluator.evaluate()
    evaluator.accumulate()
...
def train(args: argparse.Namespace) -> None:
    ...
    groups = build_layerwise_param_groups(
        ...
        pixel_embed_lr=args.pixel_embed_lr,
        gamma=args.layerwise_gamma,
    ...
    if args.resume:
        ...
        try:
            ...
            print(f"skipping optimizer/scheduler restore ({exc}); using fresh layer-wise groups")
    ...
    if args.copy_paste_disable_fraction <= 0:
        copy_paste_off_epoch = args.epochs + 1
    else:
        copy_paste_off_epoch = math.ceil(
            args.epochs * (1.0 - args.copy_paste_disable_fraction)
        )
    print(
        f"task={args.task} device={device} "
        f"train_images={len(dataset.images)} val_images={len(val_index.images)} "
        f"tiles/epoch={len(dataset)} input={args.tile_size} eval_stride={args.stride} "
        f"s2_rgb_mode={args.s2_rgb_mode} rgb_adapter={args.rgb_adapter}"
    )
    print(
        ...
        f"copy_paste_off_after_epoch={copy_paste_off_epoch}, "
        f"cascade={args.cascade}, seesaw={args.seesaw}, "
        f"layerwise_gamma={args.layerwise_gamma}"
    )
    print(
        "param groups: "
        + ", ".join(
            f"lr={group['lr']:.2e} n={len(group['params'])} wd={group['weight_decay']}"
            for group in groups
        )
    )
    print(f"parameters: trainable={trainable:,}, frozen={frozen:,}")
    eval_id_path = out_dir / "val_image_ids.json"
    if getattr(args, "eval_image_ids", None):
        args.eval_image_ids = [int(image_id) for image_id in args.eval_image_ids]
    elif args.resume and eval_id_path.exists():
        args.eval_image_ids = load_eval_image_ids(eval_id_path)
    else:
        preview = select_eval_images(val_index.images, args.val_max_images, args.seed)
        args.eval_image_ids = [int(row["id"]) for row in preview]
    eval_id_path.write_text(
        json.dumps({"image_ids": args.eval_image_ids}, indent=2),
        encoding="utf-8",
    )
    print(f"val image ids ({len(args.eval_image_ids)}): {eval_id_path}")
    config_payload = {
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        live_copy_paste = (
            args.copy_paste_probability if epoch < copy_paste_off_epoch else 0.0
        )
        dataset.copy_paste_probability = live_copy_paste
        if epoch == copy_paste_off_epoch and args.copy_paste_probability > 0:
            print(f"epoch={epoch} copy-paste disabled for final finetune")
        model.train()
        ...
        if should_eval:
            ...
            print(
                ...
                f"val_AP50_building={metrics.get('AP50_building', 0.0):.4f}"
            )
        else:
            print(
                f"epoch={epoch} loss={row['loss']:.4f} "
...
def load_for_inference(
    ...
    model = build_model(
        ...
        use_rgb_adapter=bool(checkpoint.get("rgb_adapter", getattr(args, "rgb_adapter", True))),
...
def add_common(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        ...
        action=argparse.BooleanOptionalAction,
        default=True,
        help="trainable 3→12 S2 adapter initialized to the handcrafted RGB map",
    )
    parser.add_argument(
        "--eval-ids",
        default=None,
        help="JSON list or {image_ids: [...]} reused for val/test closed-loop eval",
    )


def add_inference(parser: argparse.ArgumentParser) -> None:
    parser.add_argument("--stride", type=int, default=0, help="0 means tile-size")
...
def build_parser() -> argparse.ArgumentParser:
    ...
    train_parser = commands.add_parser(
        "train", help="train v5 Cascade+Seesaw with RGB adapter (single GPU)"
    )
    ...
    train_parser.add_argument(
        "--copy-paste-disable-fraction",
        type=float,
        default=0.1,
        help="turn copy-paste off for the last this fraction of epochs (0 keeps it on)",
    )
    ...
    train_parser.add_argument(
        ...
        type=float,
        default=2e-5,
        help="top encoder-block lr with --unfreeze-backbone; lower blocks decay by gamma",
    )
    train_parser.add_argument(
        "--pixel-embed-lr",
        type=float,
        default=5e-5,
        help="learning rate for encoder.patch_embeddings",
    )
    train_parser.add_argument(
        "--adapter-lr",
        type=float,
        default=None,
        help="RGB adapter lr; default matches --lr",
    )
    train_parser.add_argument(
        "--layerwise-gamma",
        type=float,
        default=0.8,
        help="encoder block lr decay; block l uses backbone-lr * gamma**(L-1-l)",
    )
    ...
    eval_json_parser.add_argument(
        "--eval-ids",
        default=None,
        help="JSON list or {image_ids: [...]} reused for closed-loop eval",
    )
    return parser


def load_eval_image_ids(path: str | Path) -> list[int]:
    payload = json.loads(Path(path).read_text(encoding="utf-8"))
    if isinstance(payload, dict):
        payload = payload.get("image_ids", payload.get("ids"))
    if not isinstance(payload, list) or not payload:
        raise ValueError("--eval-ids must be a non-empty JSON list or {image_ids: [...]}")
    return [int(image_id) for image_id in payload]
...
def validate_args(args: argparse.Namespace) -> None:
    ...
    if hasattr(args, "stride"):
        ...
        elif not 0 < args.stride <= args.tile_size:
            raise ValueError("expected 0 < --stride <= --tile-size (or 0 for tile-size)")
    if getattr(args, "eval_ids", None):
        args.eval_image_ids = load_eval_image_ids(args.eval_ids)
    probability_names = (
        "score_threshold",
        "mask_threshold",
        "box_nms",
    ...
    if args.command == "train":
        ...
        if args.pixel_embed_lr <= 0:
            raise ValueError("--pixel-embed-lr must be positive")
        if args.adapter_lr is not None and args.adapter_lr <= 0:
            raise ValueError("--adapter-lr must be positive")
        if not 0 < args.layerwise_gamma <= 1:
        ...
        if not 0 <= args.copy_paste_disable_fraction <= 1:
            raise ValueError("--copy-paste-disable-fraction must be in [0, 1]")
